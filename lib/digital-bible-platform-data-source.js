...
/**
 * Find the video based on the provided ids
 *
 * @param  {String} bookId    The Book id
 * @param  {Number} chapterId The number for the given chapter
 * @param  {Number} verseId   The number for the given verse
 * @return {Object}           The video based object
 * @access public
 */
DigitalBiblePlatformDataSource.prototype.findByIds = function (bookId, chapterId, verseId) {
  var deferred = new Deferred();
  var bookDetails = this.findBookDetailsByCode(bookId);
  var self = this;
  var params = {
    key: this.apiKey,
    encoding: 'mp4',
  ...
  params.chapter_id = parseInt(chapterId, 10);
  params.verse_id = parseInt(verseId, 10);
  if ((!bookDetails) || (!params.book_id) || (!params.chapter_id) || (!params.verse_id)) {
    deferred.reject({});
  } else {
    this.getRequest('video/videopath', params).then(function(results) {
      var videos = _.map(results, function(item) {
...
/**
 * Find the details for a specific Bible book based on the given code
 *
 * @param  {String} code    The code for the book you are looking for
 * @return {Object}         The details of the book
 * @access public
 */
DigitalBiblePlatformDataSource.prototype.findBookDetailsByCode = function (code) {
  return _.find(
    this.bookData,
    function(b) {
      return b.code.toLowerCase() === code.toLowerCase();
    }
  );
};
/**
 ...
 * @access public
 */
DigitalBiblePlatformDataSource.prototype.getVideoTitle = function (result, appendTitle) {
