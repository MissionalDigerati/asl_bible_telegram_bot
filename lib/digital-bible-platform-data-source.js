...
/**
 * Get the video title
 * @param  {Object}   video      The video result
 * @param  {Boolean}  appendTitle Do you want to append the title
 * @return {String}               The final title
 *
 * @access public
 */
DigitalBiblePlatformDataSource.prototype.getVideoTitle = function (video, appendTitle) {
  var title = video.book + ' ' + video.chapter_start + ':' + video.verse_start;
  if ((video.chapter_end) && (video.chapter_start != video.chapter_end)) {
    title += '-' + video.chapter_end + ':' + video.verse_end;
  } else {
    title += '-' + video.verse_end;
  }
  if (appendTitle) {
    title += ' - ' + video.title;
  }
