...
FBMessengerBot.prototype.runPassageSearch = function(messaging, passage) {
  ...
  this.dataSource.findVideos(passage).then(function(result) {
    ...
    if (videos.length == 1) {
      ...
      var videoOptions = _(options).clone();
      var messageOptions = _(options).clone();
      videoOptions.message = {
        attachment: {
          type: 'video',
          payload: {
            url: videos[0].path
      ...
      self.sendRequest('messages', videoOptions).then(function() {
        self.sendRequest('messages', messageOptions);
        deferred.resolve(true);
      });
    } else if (videos.length > 1) {
      var replyOptions = _(options).clone();
      var choices = [];
      ...
      replyOptions.message = {
        text: 'We have found more than 1 passage.  Which passage would you like?',
        quick_replies: choices
      };
      self.sendRequest('messages', replyOptions).then(function() {
        deferred.resolve(true);
      });
    } else if (result.available === false) {
      var helpOptions = _(options).clone();
      helpOptions.message = {
        attachment: {
          type: 'template',
          payload: {
            template_type: 'button',
            text: passage+' has not been translated yet. Can you help?',
            buttons: [
              {
                type: 'web_url',
              ...
              {
                type: 'web_url',
                url: 'https://deafmissions.givingfuel.com/deaf-missions-giving',
                title: 'Donate'
              }
            ]
          }
      ...
      self.sendRequest('messages', helpOptions).then(function() {
        deferred.resolve(true);
      });
    } else {
      var messageOptions = _(options).clone();
      messageOptions.message = {
        text: 'Sorry, no videos were found.  Please try another search.'
      }
      self.sendRequest('messages', messageOptions).then(function() {
        deferred.resolve(true);
      });
      options.text = '';
    }
