  ...
  return this.sendRequest('/getUpdates').then(
    function(content) {
      ...
      for (var i = 0; i < updates.length; i++) {
        ...
        if (update) {
          if ((_.has(update, 'message')) && (update.message.date > self.lastUpdated)) {
            console.log('here');
            updateTimestamps.push(update.message.date);
            promises.push(self.handleCommands(update.message));
          } else if ((_.has(update, 'inline_query')) && (_.indexOf(self.lastInlineQueriesHandled, update.id) == -1)) {
            promises.push(
              self.handleQuery(update.inline_query));
          }
        }
      }
...
TelegramBot.prototype.handleQuery = function(update) {
  ...
  if (scriptureMatch) {
    this.runSearch(update).then(function() {
      deferred.resolve(true);
    });
  } else {
    deferred.resolve(true);
 ...
 * @access public
 */
TelegramBot.prototype.runSearch = function(inlineQuery) {
  var self = this;
  var options = {
    inline_query_id: inlineQuery.id
  };
  return this.dataSource.findVideos(inlineQuery.query).then(function(results) {
    var searchResults = [];
    for (var i = 0; i < results.length; i++) {
      var title = results[i].title + ' (' + results[i].book + ' ' + results[i].chapter_start + ':' + results[i].verse_start + '-' + results[i].verse_end + ')';
      var video = {
        type: 'video',
        id: results[i].segment_order,
        video_url: results[i].path,
        title: title,
        mime_type: 'video/mp4',
        thumb_url: 'http://codemis.com/img/asl-bible-placeholder.jpg'
      };
