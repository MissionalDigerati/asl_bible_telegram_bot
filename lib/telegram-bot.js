...
TelegramBot.prototype.runCommandGet =  function(update, requestedBook) {
  ...
  return this.dataSource.findVideos(requestedBook).then(function(result) {
    ...
    if (videos.length == 1) {
      var title = self.videoTitle(videos[0], false);
      options.text = '<b>' + title + '</b>: <a href="' + videos[0].path + '">' + videos[0].title + '</a>\n\n<em>Provided by the Deaf Bible Society & Faith Comes by Hearing.</em>'
    ...
    if ((videos.length === 0) && (result.available === false)) {
      searchResults.push({
        ...
        input_message_content: {
          parse_mode: 'HTML',
          message_text: 'The ASL translation for <b>' + result.book + '</b> has yet to be completed?  Find out how you can help at <a href="https://www.deafbiblesociety.com/projects">Deaf Bible Society Projects</a>!',
