...
TelegramBot.prototype.runCommandStart =  function(update) {
  var options = {
    ...
    'I can help you find Bible passages in American Sign Language.  All you have to do is type a verse in this format: <b>Matthew 18:19-20</b>.  Are you ready to start?'
  };
...
TelegramBot.prototype.runCommandGet =  function(update, requestedBook) {
  ...
  return this.dataSource.findVideos(requestedBook).then(function(results) {
    if (results.length == 1) {
      options.text = '<b>' + requestedBook + '</b>: ' + '<a href="' + results[0].path + '">' + results[0].title + '</a>';
